...
export default function LatestPosts() {
    ...
    async function loadPosts() {
      try {
        if (!isFirebaseConfigured()) {
          ...
          setError("Firebase is not configured on the homepage.");
          return;
        }
        ...
        console.error("Homepage could not load posts", err);
        setPosts(samplePosts);
        setError(err?.message ?? "Homepage could not load Firebase posts.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return <p className="admin-status">Loading latest updates...</p>;
  }
  ...
  if (!posts.length) {
    return <p className="admin-status">No published posts found. Create a post from the admin dashboard.</p>;
  }
