...
export default function SchemeCategoryPageClient({
  categoryName,
  initialGroups,
}: {
  categoryName: string;
  ...
  const { schemes, availableCategories } = useMemo(() => {
    const allSchemes: SchemePost[] = [];

    for (const collectionName of SCHEME_COLLECTIONS) {
      const records = initialGroups[collectionName] || [];
