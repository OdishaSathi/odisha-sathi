    ...
    reminderAdmitCards.map((admitCard) => {
      ...
      return {
        ...
        lastDate: reminderInfo
          ...
          : "Date not available",
        dateLabel: reminderInfo?.label || "Exam Date",
...
function buildAdmitCardsFromGroups(
  ...
  for (const collectionName of ADMIT_CARD_COLLECTIONS) {
    for (const record of groups[collectionName] || []) {
      ...
      allAdmitCards.push({
        ...
        examDateDisplay:
          data.examDateDisplay || data.examDisplayDate || data.displayExamDate || "",
        examDate:
          data.examDate ||
    ...
    new Map(
      validAdmitCards
        ...
        .map((item) => [
          ...
          { label: getDisplayCategoryName(item), value: item },
        ])
    ).values()
  ).sort((a, b) => a.label.localeCompare(b.label));

  const admitCards = validAdmitCards
    .filter((item) => matchesSelectedSubCategory(item, subCategoryName))
    .sort((a, b) => getTimeValue(b) - getTimeValue(a));

  return { admitCards, availableCategories };
}

export default function AdmitCardSubCategoryPageClient({
  subCategoryName,
  initialGroups,
}: {
  subCategoryName: string;
  ...
  const { admitCards, availableCategories } = useMemo(
    () => buildAdmitCardsFromGroups(initialGroups, subCategoryName),
            ...
            <section className="os-side-card os-reminder-card">
              <div className="os-reminder-head">
                ...
                <ReminderShareButtons
                  ...
                  disabled={reminderAdmitCards.length === 0}
