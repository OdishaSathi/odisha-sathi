...
function getRawResultDate(item: ResultPost) {
  if (item.canonicalResultDate) return item.canonicalResultDate;
...
function mapResultRecord(record: PublicRecord, collectionName: string): ResultPost {
  ...
  return {
    ...
    resultDate: data.resultDate || data.resultDeclarationDate || data.resultDeclaredDate || "",
    resultDateDisplay: data.resultDateDisplay || data.resultDisplayDate || data.displayResultDate || "",
...
function buildInitialResults(groups: Record<string, PublicRecord[]>): ResultPost[] {
  const all = RESULT_COLLECTIONS.flatMap((name) => (groups[name] || []).map((record) => mapResultRecord(record, name)));
  return Array.from(new Map(all.filter(isResultPost).map((item) => [getResultKey(item), item])).values())
...
export default function ResultsPageClient({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const results = useMemo(() => buildInitialResults(initialGroups), [initialGroups]);
