...
function LatestSchemeStack({
  ...
  const lifecycle = getApplicationLifecycleStatus({
    startDate: getRawStartDate(scheme),
    lastDate: getRawLastDate(scheme),
    fallbackStatus: scheme.lifecycleStatus,
  });
...
function ReminderItem({ scheme }: { scheme: SchemePost }) {
  const lastDate = getLastDate(scheme);
  const urgency = getDeadlineUrgencyText(getRawLastDate(scheme));
...
function buildInitialSchemes(groups: Record<string, PublicRecord[]>): SchemePost[] {
  const collectionNames = ["posts", "schemes", "scheme", "governmentSchemes", "government-schemes"];
  const combined = collectionNames.flatMap((name) => (groups[name] || [])
    .filter((record) => isPublicListingPost({ ...(record.data || {}), category: record.data?.category || name }, record.id))
    .map((record) => mapSchemeDocument({ id: record.id, data: () => record.data }, name)));
  const unique = new Map<string, SchemePost>();
  combined.filter(isSchemePost).forEach((item) => { const key = item.slug || item.id; if (!unique.has(key)) unique.set(key, item); });
  return Array.from(unique.values()).sort((a, b) => getTimeValue(b) - getTimeValue(a));
}

export default function SchemesPageClient({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const schemes = useMemo(() => buildInitialSchemes(initialGroups), [initialGroups]);
