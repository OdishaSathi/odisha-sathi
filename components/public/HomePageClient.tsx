...
import { getDeadlineUrgencyText, getUpcomingUrgencyText } from "@/lib/reminderUrgency";
...
function getReminderUrgencyLabel(item: PostItem) {
  const reminderInfo = getReminderDateInfo(item);
  if (!reminderInfo) return "";

  return reminderInfo.label === "Last Date"
    ? getDeadlineUrgencyText(reminderInfo.rawDate)
    : getUpcomingUrgencyText(reminderInfo.rawDate);
}
...
function createPostItem(
  ...
  return {
    ...
    canonicalStartDate: data.canonicalStartDate || "",
    canonicalLastDate: data.canonicalLastDate || "",
    canonicalExamDate: data.canonicalExamDate || "",
...
function createImportantInformationItem(id: string, data: any): ImportantInformationItem {
  return {
    ...
    canonicalLastDate: data.canonicalLastDate || "",
    tickerEndDate: data.tickerEndDate || data.endDate || "",
...
function getImportantInfoExplicitEndDate(item: ImportantInformationItem) {
  const direct = item.tickerEndDate || item.canonicalLastDate;
  if (direct) return parseEndDateValue(direct);

  const matched = (item.importantDates || []).find((row) => {
    const label = String(row.label || row.type || "").toLowerCase();
    return /(last\s*date|end\s*date|closing\s*date|deadline|application\s*end)/i.test(label);
  });

  return matched?.value ? parseEndDateValue(matched.value) : null;
}

function isImportantInfoTickerActive(item: ImportantInformationItem) {
  const now = Date.now();
  const explicitEnd = getImportantInfoExplicitEndDate(item);

  if (explicitEnd) {
    const endOfDay = new Date(explicitEnd);
    endOfDay.setHours(23, 59, 59, 999);
    return now <= endOfDay.getTime();
  }

  const createdSeconds = getImportantInfoTimeValue(item);
  if (!createdSeconds) return true;

  const defaultExpiry = createdSeconds * 1000 + 7 * 24 * 60 * 60 * 1000;
...
function ReminderRow({ item }: { item: PostItem }) {
  ...
  const urgencyLabel = getReminderUrgencyLabel(item);
...
function recordsAsDocs(records: PublicRecord[] = []) {
  return records.map((item) => ({ id: item.id, data: () => item.data }));
}

function buildHomeDataFromGroups(groups: Record<string, PublicRecord[]>): HomeData {
  const postItems = createPublicPostItems(recordsAsDocs(groups.posts));
  const jobCollectionItems = createPublicPostItems(recordsAsDocs(groups.jobs), "jobs");
  const admissionCollectionItems = createPublicPostItems(recordsAsDocs(groups.admissions), "admissions");
  const admitCardItems = [
    ...createPublicPostItems(recordsAsDocs(groups.admitCards), "admit-cards"),
    ...createPublicPostItems(recordsAsDocs(groups["admit-cards"]), "admit-cards"),
    ...createPublicPostItems(recordsAsDocs(groups.admitcards), "admit-cards"),
  ];
  const resultItems = [
    ...createPublicPostItems(recordsAsDocs(groups.results), "results"),
    ...createPublicPostItems(recordsAsDocs(groups.result), "results"),
  ];
  const schemeCollectionItems = [
    ...createPublicPostItems(recordsAsDocs(groups.schemes), "schemes"),
    ...createPublicPostItems(recordsAsDocs(groups.scheme), "schemes"),
    ...createPublicPostItems(recordsAsDocs(groups.governmentSchemes), "schemes"),
    ...createPublicPostItems(recordsAsDocs(groups["government-schemes"]), "schemes"),
  ];
  const importantInformationItems = (groups.importantInformation || [])
    .map((item) => createImportantInformationItem(item.id, item.data))
    .filter((item) => item.status !== "hidden")
    .sort((a,b) => getImportantInfoTimeValue(b)-getImportantInfoTimeValue(a));
  const importantFeatured = [...importantInformationItems].filter((item) => item.isFeatured).sort((a,b) => {
    const d=(a.featuredOrder||0)-(b.featuredOrder||0); return d || getImportantInfoTimeValue(b)-getImportantInfoTimeValue(a);
  }).slice(0, IMPORTANT_INFO_TILE_LIMIT);
  const allItems = getDedupedPosts([...postItems,...jobCollectionItems,...admissionCollectionItems,...admitCardItems,...resultItems,...schemeCollectionItems]);
  const jobs=allItems.filter((i)=>i.category==="jobs").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const admissions=allItems.filter((i)=>i.category==="admissions").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const schemes=allItems.filter((i)=>i.category==="schemes").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const admitCards=allItems.filter((i)=>i.category==="admit-cards").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const results=allItems.filter((i)=>i.category==="results").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const citizenServices=allItems.filter((i)=>i.category==="citizen-services").sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const allAllowedPosts=[...jobs,...admitCards,...results,...admissions,...citizenServices].filter(isAllowedLatestPost);
  const manuallySelected=allAllowedPosts.filter((i)=>i.homeLatestSelected&&!i.homeLatestHidden).sort((a,b)=>Number(a.homeLatestOrder||999)-Number(b.homeLatestOrder||999)||getTimeValue(b)-getTimeValue(a));
  const selectedKeys=new Set(manuallySelected.map((i)=>`${i.category}-${i.slug||i.id}`));
  const automaticPosts=allAllowedPosts.filter((i)=>!i.homeLatestHidden&&!selectedKeys.has(`${i.category}-${i.slug||i.id}`)).sort((a,b)=>getTimeValue(b)-getTimeValue(a));
  const latestTiles=[...manuallySelected,...automaticPosts];
  const reminders=[...allAllowedPosts].filter(isLastDateReminderPost).sort((a,b)=>getEndDateTime(a)-getEndDateTime(b));
  return { importantFeatured, importantAll:importantInformationItems, latestTiles, jobs, admitCards, results, admissions, schemes:citizenServices, reminders };
}

export default function HomePage({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const settings = usePublicSiteSettings();
  const homeData = useMemo(() => buildHomeDataFromGroups(initialGroups), [initialGroups]);
  const loading = false;
  ...
  const visibleLatestTiles = homeData.latestTiles.slice(0, settings.latestPostsCount || LATEST_TILE_LIMIT);
    ...
    <main className="os-board-home">
      <div className="os-board-container">
        ...
        {loading ? (
          ...
          <>
            {settings.showImportantInformation ? (
              <ImportantUpdatesTicker
                items={homeData.importantAll.filter(isImportantInfoTickerActive).slice(0, 12)}
              />
            ) : null}
            ...
            <section className="os-home-bottom-layout" style={!settings.showLatestCitizenServices ? { gridTemplateColumns: "1fr" } : undefined}>
              ...
              {settings.showLatestCitizenServices ? (
