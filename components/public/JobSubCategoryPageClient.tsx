...
function matchesSelectedSubCategory(item: JobPost, selectedSubCategory: string) {
  ...
  if (subCategory === selected) return true;

  if (Array.isArray(item.subCategories)) {
    return item.subCategories.some((categoryItem) => {
      return normalizeText(categoryItem) === selected;
    });
  }
...
function CategoryJobItem({ job }: { job: JobPost }) {
  ...
  return (
    <Link href={`/post/${job.slug || job.id}`} className="os-category-job-item">
      <div>
        <h3>{job.title || "Untitled Job"}</h3>
        <p>{postName}</p>
      </div>

      <div className="os-category-job-date-grid">
        {publishedDate ? <span>Published: {publishedDate}</span> : null}
        {startDate ? <span className="os-date-green">Start Date: {startDate}</span> : null}
        {lastDate ? <span className="os-date-red">Last Date: {lastDate}</span> : null}
        <PublicStatusBadge status={lifecycle} />
      </div>
    </Link>
  );
...
function buildJobsFromRecords(records: PublicRecord[], subCategoryName: string): JobPost[] {
  return records
    .filter((record) => isPublicListingPost(record.data, record.id))
    .map((record) => {
      ...
      return {
        ...
        quickInfoPanels: Array.isArray(data.jobInfoPanels)
          ? data.jobInfoPanels
          : Array.isArray(data.quickInfoPanels)
          ? data.quickInfoPanels
          : [],
        jobInfoPanels: Array.isArray(data.jobInfoPanels) ? data.jobInfoPanels : [],
...
export default function JobSubCategoryPageClient({
  subCategoryName,
  initialRecords,
}: {
  ...
  const jobs = useMemo(
    () => buildJobsFromRecords(initialRecords, subCategoryName),
    [initialRecords, subCategoryName]
  );
    ...
    <main className="os-category-page">
      <div className="os-category-container">
        <section className="os-category-top">
          <Link href="/jobs" className="os-back-link">
            ← Back to Jobs
          </Link>

          <p>ODISHA SATHI JOBS</p>
          <h1>{subCategoryName}</h1>
          <span>Latest jobs listed under {subCategoryName}.</span>
        </section>
            ...
            <section className="os-list-section">
              ...
              {latestJobs.length === 0 ? (
                <p className="os-list-status">
                  No latest jobs found in this category.
                </p>
              ) : (
              ...
              <div className="os-list-section-head">
                ...
                <label className="os-job-qualification-filter">
                  ...
                  <select
                    ...
                    aria-label={`Filter ${subCategoryName} by qualification`}
                  >
              ...
              {filteredJobs.length === 0 ? (
                <p className="os-list-status">
                  {qualificationFilter === "all"
                    ...
                    : "No jobs found for this qualification in this category."}
                </p>
              ) : (
                <>
                  <div className="os-category-job-list">
                    {visibleJobs.map((job) => (
                      <CategoryJobItem key={job.id} job={job} />
                    ))}
                  </div>
              ...
              <div className="os-side-category-list">
                {JOB_SUB_CATEGORIES.map((item) => (
                  <Link
                    ...
                    className={
                      normalizeText(item) === normalizeText(subCategoryName)
                        ? "os-active-category"
                        : ""
                    }
                  >
            ...
            <section className="os-side-card os-reminder-card">
              <div className="os-reminder-head">
                ...
                <ReminderShareButtons
                  ...
                  disabled={reminderJobs.length === 0}
              ...
              {reminderJobs.length === 0 ? (
                <p className="os-side-status">
                  No job deadline in this category within the next 7 days.
                </p>
              ) : (
      ...
      <style jsx global>{`
        ...
        .os-category-job-list {
          display: grid;
          gap: 10px;
          padding: 14px;
        }

        .os-category-job-item {
          display: grid;
        ...
        .os-category-job-item:hover {
          background: #fff7ed;
          border-color: #fed7aa;
          transform: translateY(-1px);
        }

        .os-category-job-item h3 {
          margin: 0;
          color: #1d4ed8;
          font-size: 15px;
          line-height: 1.35;
        ...
        .os-category-job-item:hover h3 {
          color: #ea580c;
        }

        .os-category-job-item p {
          margin: 5px 0 0;
          color: #475569;
          font-size: 13px;
          line-height: 1.35;
        ...
        .os-category-job-date-grid span {
          color: #64748b;
          font-size: 12px;
          line-height: 1.2;
          font-weight: 900;
        ...
        .os-category-job-date-grid .os-date-green {
          color: #166534;
        }

        .os-category-job-date-grid .os-date-red {
          color: #dc2626;
        }
