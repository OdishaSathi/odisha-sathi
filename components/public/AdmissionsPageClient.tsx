...
import { getDeadlineUrgencyText } from "@/lib/reminderUrgency";
import PublicStatusBadge from "@/components/public/PublicStatusBadge";
import { getApplicationLifecycleStatus } from "@/lib/publicLifecycle";
import {
  DEFAULT_ADMISSION_SUBCATEGORIES,
  cleanAdmissionCategoryList,
  getCanonicalAdmissionCategory,
  isScholarshipAdmissionCategory,
  normalizeAdmissionCategoryKey,
} from "@/lib/admissionCategories";

const ADMISSION_SUB_CATEGORIES = DEFAULT_ADMISSION_SUBCATEGORIES;
...
function LatestAdmissionStack({
  ...
  const lifecycle = getApplicationLifecycleStatus({
    startDate: getRawStartDate(admission),
    lastDate: getRawLastDate(admission),
    fallbackStatus: admission.lifecycleStatus,
  });
...
function ReminderItem({ admission }: { admission: AdmissionPost }) {
  const lastDate = getLastDate(admission);
  const urgency = getDeadlineUrgencyText(getRawLastDate(admission));
...
const normalizeCategoryKey = normalizeAdmissionCategoryKey;
...
function mapAdmissionRecord(record: PublicRecord, sourceCollection = "posts"): AdmissionPost {
  const data = record.data || {};
  return {
    id: record.id, title: data.title || data.schemeName || "", slug: data.slug || "",
    content: data.content || data.description || "", category: data.category || "", subCategory: data.subCategory || "",
    subCategories: Array.isArray(data.subCategories) ? data.subCategories : data.subCategory ? [data.subCategory] : [],
    admissionCategory: data.admissionCategory || "", admissionCategories: Array.isArray(data.admissionCategories) ? data.admissionCategories : [],
    categoryName: data.categoryName || "", categorySlug: data.categorySlug || "", subCategorySlug: data.subCategorySlug || "",
    categories: Array.isArray(data.categories) ? data.categories : [], courseName: data.courseName || data.course || data.admissionCourse || "",
    admissionName: data.admissionName || "", programmeName: data.programmeName || "", programName: data.programName || "", className: data.className || "",
    department: data.department || "", organization: data.organization || "",
    startDateDisplay: data.startDateDisplay || data.applicationStartDateDisplay || "",
    startDate: data.startDate || data.admissionStartDate || data.applicationStartDate || data.openingDate || "",
    admissionStartDate: data.admissionStartDate || "", applicationStartDate: data.applicationStartDate || "",
    applicationStartDateDisplay: data.applicationStartDateDisplay || "", openingDate: data.openingDate || "",
    lastDateDisplay: data.lastDateDisplay || "", lastDate: data.lastDate || data.admissionLastDate || data.applicationLastDate || data.applicationEndDate || data.closingDate || data.endDate || "",
    admissionLastDate: data.admissionLastDate || "", applicationLastDate: data.applicationLastDate || "", applicationEndDate: data.applicationEndDate || "",
    closingDate: data.closingDate || "", endDate: data.endDate || "", importantDates: data.importantDates || [], canonicalStartDate: data.canonicalStartDate || "", canonicalLastDate: data.canonicalLastDate || "", lifecycleStatus: data.lifecycleStatus || "", createdAt: data.createdAt || null, sourceCollection,
  } as AdmissionPost;
}

function buildInitialAdmissions(groups: Record<string, PublicRecord[]>): AdmissionPost[] {
  return ["posts", "admissions", "admission"]
    .flatMap((name) => (groups[name] || []).map((record) => mapAdmissionRecord(record, name)))
    .filter((item) => {
      if (!["admissions", "admission"].includes(String(item.category || "").toLowerCase()) && !["admissions", "admission"].includes(String((item as any).sourceCollection || "").toLowerCase())) return false;
      const categoryValues = getCategoryValues(item);
      const scholarshipLike = categoryValues.some(isScholarshipAdmissionCategory) || /scholarship/i.test(`${item.title || ""} ${item.content || ""}`);
      return !scholarshipLike;
    })
    .sort((a, b) => getTimeValue(b) - getTimeValue(a));
}

export default function AdmissionsPageClient({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const admissions = useMemo(() => buildInitialAdmissions(initialGroups), [initialGroups]);
  const availableCategories = useMemo(
    () =>
      Array.from(
        new Map(
          cleanAdmissionCategoryList(
            admissions.flatMap((item) => getCategoryValues(item))
  ...
  const sideCategories = Array.from(
    new Map(
      cleanAdmissionCategoryList([...ADMISSION_SUB_CATEGORIES, ...availableCategories])
    ...
    <main className="os-list-page">
      <div className="os-list-container">
        <section className="os-list-top">
          <h1>
            ODISHA SATHI ADMISSIONS (Find all admission updates on this page)
          </h1>
        </section>
