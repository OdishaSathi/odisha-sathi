...
import {
  JOB_QUALIFICATION_FILTERS,
  matchesJobQualification,
  type JobQualificationFilter,
} from "@/lib/jobQualificationFilter";
...
function LatestJobStack({ job, index }: { job: JobPost; index: number }) {
  ...
  const lifecycle = getApplicationLifecycleStatus({
    startDate: getRawStartDate(job),
    lastDate: getRawLastDate(job),
    fallbackStatus: job.lifecycleStatus,
  });
...
function ReminderItem({ job }: { job: JobPost }) {
  const lastDate = getLastDate(job);
  const urgency = getDeadlineUrgencyText(getRawLastDate(job));
...
function buildJobsFromRecords(records: PublicRecord[]): JobPost[] {
  return records.filter((item) => isPublicListingPost(item.data, item.id)).map((item) => {
    const data = item.data;
    return { id:item.id, title:data.title||"", slug:data.slug||"", content:data.content||data.description||"", category:data.category||"",
      subCategory:data.subCategory||"", subCategories:Array.isArray(data.subCategories)?data.subCategories:(data.subCategory?[data.subCategory]:[]),
      department:data.department||"", organization:data.organization||"", postName:data.postName||data.jobPostName||data.post||"",
      postNames:Array.isArray(data.postNames)?data.postNames:[], startDate:data.startDate||data.applicationStartDate||data.applicationOpenDate||data.openingDate||"",
      applicationStartDate:data.applicationStartDate||"", applicationOpenDate:data.applicationOpenDate||"", openingDate:data.openingDate||"",
      lastDate:data.lastDate||data.applicationLastDate||data.applicationEndDate||data.closingDate||data.endDate||"",
      applicationLastDate:data.applicationLastDate||"", applicationEndDate:data.applicationEndDate||"", closingDate:data.closingDate||"", endDate:data.endDate||"",
      importantDates:data.importantDates||[], quickInfoPanels:Array.isArray(data.jobInfoPanels)?data.jobInfoPanels:(Array.isArray(data.quickInfoPanels)?data.quickInfoPanels:[]), jobInfoPanels:Array.isArray(data.jobInfoPanels)?data.jobInfoPanels:[], qualification:data.qualification||"", canonicalStartDate:data.canonicalStartDate||"", canonicalLastDate:data.canonicalLastDate||"", lifecycleStatus:data.lifecycleStatus||"", createdAt:data.createdAt||null };
  }).filter((item) => item.category === "jobs").sort((a,b) => getTimeValue(b)-getTimeValue(a));
}

export default function JobsPage({ initialRecords }: { initialRecords: PublicRecord[] }) {
  const jobs = useMemo(() => buildJobsFromRecords(initialRecords), [initialRecords]);
  const [visibleCount, setVisibleCount] = useState(POSTS_PER_PAGE);
  const [qualificationFilter, setQualificationFilter] =
    useState<JobQualificationFilter>("all");
  const loading = false;

  const latestJobs = jobs.slice(0, LATEST_STACK_COUNT);
  const filteredJobs = useMemo(
    () => jobs.filter((job) => matchesJobQualification(job, qualificationFilter)),
    [jobs, qualificationFilter]
  );
  const visibleJobs = filteredJobs.slice(0, visibleCount);
  const canViewMore = visibleCount < filteredJobs.length;
  ...
  return (
    <main className="os-list-page">
          ...
          <div className="os-jobs-main">
            ...
            <section className="os-list-section">
              <div className="os-list-section-head">
                ...
                <label className="os-job-qualification-filter">
                  <span>Qualification</span>
                  <select
                    value={qualificationFilter}
                    onChange={(event) => {
                      setQualificationFilter(
                        event.target.value as JobQualificationFilter
                      );
                      setVisibleCount(POSTS_PER_PAGE);
                    }}
                    aria-label="Filter jobs by qualification"
                  >
                    {JOB_QUALIFICATION_FILTERS.map((option) => (
              ...
              {loading ? (
                ...
                <p className="os-list-status">
                  {qualificationFilter === "all"
                    ? "No jobs found."
                    : "No jobs found for this qualification."}
                </p>
              ) : (
                <>
      ...
      <style jsx global>{`
        ...
        .os-job-qualification-filter select {
          min-height: 34px;
          max-width: 190px;
        ...
        .os-job-qualification-filter select:focus {
          border-color: #93c5fd;
