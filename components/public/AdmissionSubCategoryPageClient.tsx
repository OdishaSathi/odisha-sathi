...
function isAdmissionPost(item: AdmissionPost) {
  ...
  if (!["admissions", "admission"].includes(category) && !["admissions", "admission"].includes(sourceCollection)) return false;
  const categoryValues = getCategoryValues(item);
  return !categoryValues.some(isScholarshipAdmissionCategory) && !getSearchText(item).includes("scholar");
}
...
function buildAdmissionsFromGroups(
  ...
  for (const collectionName of ADMISSION_COLLECTIONS) {
    for (const record of groups[collectionName] || []) {
      ...
      allAdmissions.push({
        ...
        admissionName:
          data.admissionName || data.programName || data.programmeName || "",
        department: data.department || "",
        institute: data.institute || data.institution || "",
        organization: data.organization || "",
        startDateDisplay:
          data.startDateDisplay || data.applicationStartDateDisplay || "",
        startDate:
          data.startDate ||
        ...
        applicationStartDateDisplay: data.applicationStartDateDisplay || "",
  ...
  const availableCategories = cleanAdmissionCategoryList(
    validAdmissions.flatMap((item) => getCategoryValues(item))
  );

  const canonicalSubCategoryName = getCanonicalAdmissionCategory(subCategoryName);
  const admissions = validAdmissions
    .filter((item) => matchesSelectedSubCategory(item, canonicalSubCategoryName))
    .sort((a, b) => getTimeValue(b) - getTimeValue(a));

  return { admissions, availableCategories };
}

export default function AdmissionSubCategoryPageClient({
  subCategoryName,
  initialGroups,
}: {
  subCategoryName: string;
  ...
  const displaySubCategory = getCanonicalAdmissionCategory(subCategoryName);
  const { admissions, availableCategories } = useMemo(
    () => buildAdmissionsFromGroups(initialGroups, displaySubCategory),
    [initialGroups, displaySubCategory]
  );
    ...
    <main className="os-category-page">
      <div className="os-category-container">
        <section className="os-category-top">
          ...
          <span>Latest admissions listed under {displaySubCategory}.</span>
        </section>
          ...
          <div className="os-category-main">
            <section className="os-list-section">
              <div className="os-list-section-head">
                <h2>Latest {displaySubCategory}</h2>
              </div>
                ...
                {sideCategories.map((item) => (
                  <Link
                    ...
                    className={
                      ...
                      normalizeCategoryKey(displaySubCategory)
                        ? "os-active-category"
                        : ""
                    }
                  >
            ...
            <section className="os-side-card os-reminder-card">
              <div className="os-reminder-head">
                ...
                <ReminderShareButtons
                  ...
                  disabled={reminderAdmissions.length === 0}
