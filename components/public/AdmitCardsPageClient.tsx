...
import { getUpcomingUrgencyText } from "@/lib/reminderUrgency";
import PublicStatusBadge from "@/components/public/PublicStatusBadge";
import { getExamLifecycleStatus } from "@/lib/publicLifecycle";
...
function getRawExamDate(item: AdmitCardPost) {
  const directDate = item.canonicalExamDate || item.examDate || item.testDate || "";
...
function LatestAdmitCardStack({
  ...
  const lifecycle = getExamLifecycleStatus({
    examDate: getRawExamDate(admitCard),
    admitCardDate: getRawAdmitCardDate(admitCard),
    fallbackStatus: admitCard.lifecycleStatus,
  });
...
function ReminderItem({ admitCard }: { admitCard: AdmitCardPost }) {
  const examDate = getExamDate(admitCard);
  const urgency = getUpcomingUrgencyText(getRawExamDate(admitCard));
...
function buildInitialAdmitCards(groups: Record<string, PublicRecord[]>): AdmitCardPost[] {
  const collectionNames = ["posts", "admitCards", "admit-cards", "admitcards"];
  const combined = collectionNames.flatMap((name) => (groups[name] || []).map((record) => mapAdmitCardDocument({ id: record.id, data: () => record.data }, name)));
  const unique = new Map<string, AdmitCardPost>();
  combined.filter(isAdmitCardPost).forEach((item) => { const key = item.slug || item.id; if (!unique.has(key)) unique.set(key, item); });
  return Array.from(unique.values()).sort((a, b) => getTimeValue(b) - getTimeValue(a));
}

export default function AdmitCardsPageClient({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const admitCards = useMemo(() => buildInitialAdmitCards(initialGroups), [initialGroups]);
