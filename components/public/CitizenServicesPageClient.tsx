...
function buildInitialData(groups?: Record<string, PublicRecord[]>) {
  ...
  })).filter((item) => isPublicListingPost(item, item.id)).sort((a,b) => getTimeValue(b)-getTimeValue(a));
  ...
  return {
    ...
    latestPosts: allPosts.filter((item) => item.category !== "tools" && item.category !== "schemes").slice(0,8),
  };
}

export default function CitizenServicesPage({ initialGroups }: { initialGroups: Record<string, PublicRecord[]> }) {
  const initial = useMemo(() => buildInitialData(initialGroups), [initialGroups]);
  const categories = initial.categories;
  const services = initial.services;
  const latestPosts = initial.latestPosts;
    ...
    <main className="citizen-page">
      <div className="citizen-container">
        ...
        <section className="citizen-all-section">
          ...
          <div className="citizen-service-tools">
            <label className="citizen-service-search">
              <span className="sr-only">Search Citizen Services</span>
              <input
                type="search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search service, document or eligibility..."
            ...
            {query ? (
              <span className="citizen-search-count">
          ...
          {loading ? (
            ...
            <p className="citizen-status">
              No Citizen Services match “{query.trim()}”. Try a service name,
              document, eligibility term or category.
            </p>
          ) : (
            <>
